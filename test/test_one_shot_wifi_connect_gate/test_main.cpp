...
void test_218_arm_once_and_duplicate_rejected(){Transport t;algaguard::PhysicalTestSessionInstaller i;TEST_ASSERT_TRUE(i.install(t,kSession,kDevice,kToken,200,1));algaguard::PhysicalSessionControlProtocol p;auto f=armFrame(100);TEST_ASSERT_EQUAL_INT(static_cast<int>(algaguard::PhysicalSessionControlAck::kWifiTestArmed),static_cast<int>(p.ingest(t,i,f.data(),f.size(),2)));TEST_ASSERT_EQUAL_INT(static_cast<int>(algaguard::PhysicalSessionControlAck::kWifiTestAlreadyActive),static_cast<int>(p.ingest(t,i,f.data(),f.size(),3)));}

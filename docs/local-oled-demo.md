...
visible highlighted row; Up/Down (GPIO4/GPIO5) move the highlight, Select
(GPIO6) enters the highlighted screen, and Back (GPIO7) returns to the main
menu from anywhere -- all through the existing non-blocking debounce
boundary. The menu items are Temperature/pH, Light, Nutrients, Device
status, Network, and About (this profile has no QR pairing, so no "Pair
Device" item -- see `esp32-s3-dev-qr-onboarding-demo` below for that). Full
navigation map: `docs/oled-navigation.md`. Status text explicitly says
...
simulation pages and BLE advertising while adding a "Pair Device" main-menu
item leading to the `Scan to add`, QR ready, and QR-expired screens. A QR
invitation is public, short-lived, and one-time; it contains no Wi-Fi,
session, account, key, certificate, or cloud secret. This combined profile may initialize the Wi-Fi runtime but cannot start

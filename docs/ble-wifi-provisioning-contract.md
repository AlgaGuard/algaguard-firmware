...
The same credential response supplies the authoritative public MQTT hostname,
port, server name, keepalive, and session-expiry policy. The QR demo then uses
the issued client certificate for mutual-TLS MQTT, subscribes only to its
per-device command and telemetry-ack topics, and publishes QoS 1 telemetry only
after an authorized `APPLY_PROFILE_CONFIGURATION` command installs an active
profile in RAM. One application acknowledgement is required before the next
batch, with three bounded retries. These samples are generated by the local
demo generator and always carry `qualityFlags=[SIMULATED]` plus
`simulationScenario=device-local-demo`; they are ESP32-originated demo data,
not physical sensor readings. No MQTT password or shared device secret exists.

## Non-goals

This foundation does not implement pairing, bonding, Wi-Fi scanning, OTA,
hardware flashing, eFuse operations, or cloud deployment. Wi-Fi credential
persistence and production device-key protection remain explicitly deferred.
The QR demo's HTTPS bootstrap and mTLS MQTT path are development-only and do
not change those production non-goals.
